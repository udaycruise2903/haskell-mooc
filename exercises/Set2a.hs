...
takeFinal n xs = if length xs < n
                 then xs
                      ...
                      where result = take n (reverse xs)
...
palindromify str = if isPalindrome str then str else str
...
-- safeDiv x y = Maybe a where a = x `div` y
...
eitherDiv x 0 = Left (show x ++ "/" ++ show 0) :: Either String Integer 
eitherDiv x y = Right (x `div` y)  :: Either String Integer 
