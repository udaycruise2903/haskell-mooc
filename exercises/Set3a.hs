...
mapMaybe f x = Just(f (extractValue x))

extractValue (Just y) = y
{--
mapMaybe f x
   | f Nothing == Nothing
   | f (Just x) == Just(length (onlyX (Just x)))
   | otherwise = "error"
-}
------------------------------------------------------------------------------
...
mapMaybe2 f x y = Just(f (extractValue x)(extractValue y))
...
capitalize = todo
{-
capitalize str = capitalizeFirst str

capitalizeFirst xs = toUpper(head(head (words xs)))
-}
------------------------------------------------------------------------------
...
while check update value = if check (value) 
                           then while check update (a)
                           else value
                           where a = (update(value))
...
joinToLength n xs = [i++p | i <- xs, p <- xs, length(i++p)==n]
...
sumRights = todo
--sumRights xs = [x|x <- xs, isRight xs]
...
--interpreter commands = read (z : interpreter zs)
