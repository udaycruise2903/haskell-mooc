...
--itemCount :: ShoppingEntry -> Int
--itemPrice :: ShoppingEntry -> Double
...
getName  (MkPerson pName pAge) = pName

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (MkPerson pName pAge) = pAge

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (MkPerson pName pAge) = MkPerson name pAge

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (MkPerson pName pAge) = MkPerson pName age
...
study (NthYear x) = if x >= 0 && x <= 6 then (NthYear (x+1)) else Graduated
...
data UpDown = PlusCounter Int | MinusCounter Int
...
get :: UpDown -> Int
get (PlusCounter x) = x
get (MinusCounter x) = x

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (PlusCounter x) = PlusCounter (x+1)
tick (MinusCounter x) = MinusCounter (x-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (PlusCounter x) = (MinusCounter x)
toggle (MinusCounter x) = (PlusCounter x)
...
--rgb (Mix (rgb (Color)) (rgb (Color))) = Color + Color
--rgb (Invert Red) =   
...
toList = todo
--toList (Pair k v (KeyVals k v)) = [(k,v)]
...
fromNat :: Nat -> Int
fromNat (PlusOne Nat) = 1 + fromNat (PlusOne Nat)
